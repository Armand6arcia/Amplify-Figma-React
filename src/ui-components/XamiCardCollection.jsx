...
import { listXamiCards } from "../queries/queries";
import XamiCard from "./XamiCard";
...
export default function XamiCardCollection(props) {
    ...
    while ((newCache.length < cacheUntil || !isPaginated) && newNext != null) {
      ...
      const result = (
        await client.graphql({
          query: listXamiCards.replaceAll("__typename", ""),
          variables,
        })
      ).data.listXamiCards;
  ...
  return (
    <div>
      <Collection
        ...
        {...getOverrideProps(overrides, "XamiCardCollection")}
        {...rest}
      >
        {(item, index) => {
          if (loading) {
